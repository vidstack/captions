...
it('should parse response', async () => {
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      const lines = [
        ...
        '\n\n\n',
        'Dialogue: Agent 327,00:00:12.720,',
        '00:00:15.120,',
        "This is 327, I'm going in\n",
        'Dialogue: Agent 327,00:00:15.300,00:00:16.080,',
        'Good luck',
        ', Agent\n',
        '\n',
      ];
  ...
  expect(cues[0].text).toBe("<v Agent 327>This is 327, I'm going in");

  expect(cues[1].startTime).toBe(15.3);
  expect(cues[1].endTime).toBe(16.08);
  expect(cues[1].text).toBe('<v Agent 327>Good luck, Agent');
});
